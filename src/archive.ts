...
import type { ArchiveConfig, RunRecord, RunStatus } from './models.js';
...
function safePayloadPrefix(id: string): string {
  return id.replace(/[^A-Za-z0-9._-]/g, '_');
}

export function archiveDatabasePath(cwd: string = process.cwd()): string {
  return path.join(cwd, '.codeck', 'archive.sqlite3');
}

function fallbackPath(cwd: string): string {
  return path.join(cwd, '.codeck', 'archive.json');
}

function ensureArchiveDirs(cwd: string): void {
...
function openSqlite(cwd: string): SqliteDatabase | null {
  try {
    ...
    const { DatabaseSync } = require('node:sqlite') as { DatabaseSync: new (filePath: string) => SqliteDatabase };
...
function readFallback(cwd: string): RunRecord[] {
...
function writeFallback(cwd: string, records: RunRecord[]): void {
  ensureArchiveDirs(cwd);
  atomicWrite(fallbackPath(cwd), JSON.stringify(records, null, 2));
}

function defaultStatus(run: RunRecord): RunStatus {
...
const BUILTIN_REDACTIONS: Array<{ kind: string; pattern: RegExp; replacement: string }> = [
...
function redactText(value: string, config: ArchiveConfig): { value: string; count: number; kinds: string[] } {
  if (!config.redact || !value) return { value, count: 0, kinds: [] };
...
function redactRun(run: RunRecord, config: ArchiveConfig): RunRecord {
  const result = JSON.parse(JSON.stringify(run)) as RunRecord;
  result.archiveVersion = ARCHIVE_VERSION;
  result.status = defaultStatus(result);
  result.updatedAt ||= nowIso();
  const redactions: Array<{ field: string; count: number; kinds: string[] }> = [];
  const fields = ['task', 'prompt', 'invocationPrompt', 'contextSnapshot', 'output', 'partialOutput', 'stderr', 'error'] as const;
  for (const field of fields) {
    const value = result[field];
    if (typeof value !== 'string' || !value) continue;
    const redacted = redactText(value, config);
    result[field] = redacted.value as never;
    if (redacted.count) redactions.push({ field, count: redacted.count, kinds: redacted.kinds });
  }
  result.redactions = redactions;
  return result;
}

export function redactRunForStorage(run: RunRecord, config: ArchiveConfig): RunRecord {
  return redactRun(run, config);
}

function externalizePayloads(cwd: string, run: RunRecord, config: ArchiveConfig): RunRecord {
  ...
  if (Object.keys(refs).length) result.payloadRefs = refs;
  return result;
}

function cleanupPayloads(cwd: string, runId: string, refs: RunRecord['payloadRefs'] = {}): void {
...
function hydratePayloads(cwd: string, run: RunRecord): RunRecord {
  const result = JSON.parse(JSON.stringify(run)) as RunRecord;
  for (const [field, ref] of Object.entries(result.payloadRefs || {})) {
    const fullPath = path.resolve(cwd, ref.path);
    const payloadRoot = path.resolve(archiveDir(cwd), 'payloads');
    if (!fullPath.startsWith(`${payloadRoot}${path.sep}`) || !fs.existsSync(fullPath)) continue;
    (result as unknown as Record<string, unknown>)[field] = fs.readFileSync(fullPath, 'utf8');
  }
  return result;
}
...
function sqliteRecord(row: unknown): RunRecord | null {
  if (!row || typeof row !== 'object') return null;
  const value = (row as ArchiveRow).record_json;
...
export function archiveRun(cwd: string, run: RunRecord, config: ArchiveConfig): RunRecord {
...
export function getArchivedRun(cwd: string, runId: string): RunRecord | null {
  const db = openSqlite(cwd);
  if (db) {
    try {
...
export function listArchivedRuns(cwd: string, filter: ArchiveFilter = {}): RunRecord[] {
  ...
  if (db) {
    try {
      ...
      if (filter.executor) { clauses.push('executor = ?'); values.push(filter.executor); }
      if (filter.curated !== undefined) { clauses.push('curated = ?'); values.push(filter.curated ? 1 : 0); }
...
export function updateArchiveMetadata(cwd: string, runId: string, update: ArchiveMetadataUpdate, config: ArchiveConfig): RunRecord | null {
...
export function deleteArchivedRun(cwd: string, runId: string): boolean {
  const current = getArchivedRun(cwd, runId);
  if (!current) return false;
  for (const ref of Object.values(current.payloadRefs || {})) {
    const fullPath = path.resolve(cwd, ref.path);
    const payloadRoot = path.resolve(archiveDir(cwd), 'payloads');
    if (fullPath.startsWith(`${payloadRoot}${path.sep}`) && fs.existsSync(fullPath)) fs.unlinkSync(fullPath);
  }
  const db = openSqlite(cwd);
  if (db) {
    try {
...
export function curateArchivedRun(cwd: string, runId: string, update: ArchiveMetadataUpdate, config: ArchiveConfig): RunRecord | null {
  return updateArchiveMetadata(cwd, runId, { ...update, curated: update.curated ?? true }, config);
}

export function exportArchive(cwd: string, format: 'json' | 'markdown' = 'json', filter: ArchiveFilter = {}): string {
  ...
  for (const record of records) {
    ...
    if (record.output) lines.push('### Output', '', record.output, '');
    if (record.note) lines.push('### Note', '', record.note, '');
  }
  return lines.join('\n').trim() + '\n';
}

export function importLegacyRuns(cwd: string, config: ArchiveConfig): number {
