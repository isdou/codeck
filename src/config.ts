...
const DEFAULT_EXECUTORS: Record<string, ExecutorProfile> = {
  ...
  gemini: {
    agent: 'antigravity',
    role: 'gemini_specialist',
    description: 'General Gemini specialist through the maintained Antigravity/Agy CLI path.',
    allowed_modes: ['ask', 'subagent', 'delegate', 'compare'],
    read_files: true,
    write_files: false,
    run_shell: false,
  ...
  gemini_cli: {
    agent: 'gemini',
    role: 'gemini_cli_legacy',
    description: 'Legacy Gemini CLI path for enterprise or API-key users.',
...
export function getCodeckDir(cwd: string = process.cwd()): string {
...
- Current product state and confirmed decisions

## Build
- Technology stack, architecture, modules, and folder structure

This file is automatically generated by Codeck. Edit it once so named specialists do not need the product explained again.
`, 'utf8');
  }
...
## Product & Brand
- Voice, terminology, claims, and expressions to avoid
- Platform, audience, format, and length constraints

## Build
- Technical constraints, design preferences, code style, and things to avoid

This file is automatically generated by Codeck. Edit it to tell named specialists what they must respect.
`, 'utf8');
  }
...
export function runDoctor(cwd: string = process.cwd()): { configExists: boolean; initialized: boolean; agents: DoctorResult[]; executors: string[] } {
  const initialized = initCodeck(cwd).created;
  ...
  return {
    ...
    agents: Object.entries(config.agents).map(([agentName, agent]) => {
      ...
      return {
        ...
        requiresConsent: probe.requiresConsent,
      };
    }),
