...
import { cancelRouteTask, compareExecutors, createHandoff, CodeckError, curateRun, deleteRun, exportRuns, getRun, importRuns, listExecutors, listRuns, pickExecutor, publicRun, replayRun, routeTask, waitForRun, } from "./router.js";
...
export async function startMcpServer() {
    const server = new Server({ name: "codeck-mcp", version: VERSION }, { capabilities: { tools: {} } });
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
        ...
        try {
            switch (name) {
                case "route_task": {
                    const { fullContext, projectPath, ...input } = args;
                ...
                case "pick_executor": {
                    const { task, mode = 'ask', projectPath } = args;
                ...
                case "get_run": {
                    const run = getRun(args.runId, resolveProjectPath(args.projectPath));
                    if (!run)
                        return text('No Codeck run found.');
                    return text(JSON.stringify(args.includeContent ? run : publicRun(run), null, 2));
                }
                case "wait_run": {
                    ...
                    const run = await waitForRun(runId, resolveProjectPath(args.projectPath), timeoutMs);
                    return text(run ? JSON.stringify(publicRun(run), null, 2) : 'No Codeck run found.');
                }
                case "cancel_run": {
                    const run = cancelRouteTask(String(args.runId || ''), resolveProjectPath(args.projectPath));
                ...
                case "delete_run": {
                    ...
                    const deleted = deleteRun(String(args.runId || ''), resolveProjectPath(args.projectPath));
                ...
                case "import_runs": {
                    return text(JSON.stringify({ imported: importRuns(resolveProjectPath(args.projectPath)) }, null, 2));
                }
                case "replay_run": {
                    const value = args;
                ...
                case "list_executors": {
                    return text(listExecutors(resolveProjectPath(args.projectPath)));
                }
                case "doctor": {
                    return text(JSON.stringify(runDoctor(resolveProjectPath(args.projectPath)), null, 2));
                }
                case "ask_agent": {
                    const { agent, prompt, projectPath } = args;
                ...
                case "compare_agents": {
                    const { agents, prompt, projectPath } = args;
                ...
                case "get_last_output": {
                    return text(getRun(undefined, resolveProjectPath(args.projectPath))?.output || 'No output recorded yet.');
                }
                default:
