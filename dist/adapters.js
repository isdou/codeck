...
export function getPricing(modelOrAgent) {
...
export function calculateUsage(promptText, outputText, modelOrAgent, exactPromptTokens, exactCompletionTokens) {
...
function buildAntigravityPrompt(input, materializedPrompt) {
...
export function prepareAdapterRequest(input) {
    ...
    if (adapter === 'antigravity')
        return buildAntigravityPrompt(input, materializedPrompt);
...
function applyPromptArgs(agent, defaults, prompt) {
...
function spawnPrompt(agent, promptArgs, invocationPrompt, usagePrompt, cwd, env = {}, options = {}) {
...
function makeCliAdapter(name, defaults, capabilities, env = {}) {
    return {
        name,
        capabilities,
        probe(agent) {
        ...
        invoke(input) {
            ...
            return spawnPrompt(input.agent, promptArgs, prepared.invocationPrompt, prepared.prompt, input.cwd || process.cwd(), env, { onProgress: input.onProgress, signal: input.signal }).then((result) => ({
                ...result,
...
export function extractGeminiImageParts(data) {
...
function mimeForImageFile(filePath) {
...
function geminiImageInputParts(files, cwd = process.cwd()) {
...
export function getAdapter(name) {
    switch ((name || 'generic').toLowerCase()) {
        ...
        case 'antigravity':
            return makeCliAdapter('antigravity', (agent) => [
